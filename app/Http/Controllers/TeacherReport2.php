...
class TeacherReport2 extends Controller  
{
 ...
 public function reportPdfT2()
{
   $users = DB::table('users')
            ...
            ->get();
   $pdf  = PDF::loadview('TeacherReport2', ['users' => $users]);
   //$pdf  = PDF::loadview('reports', ['users' => $users],['subjects' => $subjects]); 
   // $pdf  = PDF::loadview('reports', ['subjects' => $subjects]); 
   

   return $pdf->download('TeacherReport2.pdf');
}

  public function reportT2()
    {//$users = DB::table('users')->get();
    //$subjects = DB::table('subjects')->get();
       // $users = DB::table('users')->get();
		...
		//$subject = Subject::all();
       // return $subject;
       return view('TeacherReport2', ['users' => $users]);
	}
}
